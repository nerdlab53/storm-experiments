...
from sub_models.attention_blocks import PositionalEncoding1D, AttentionBlock, AttentionBlockKVCache, AttentionBlockProgressive, AttentionBlockKVCacheProgressive
...
class StochasticTransformerKVCacheProgressive(nn.Module):
    def __init__(self, stoch_dim, action_dim, feat_dim, num_layers, num_heads, max_length, dropout, use_progressive_in_kv=False, use_mild_decay_in_kv=False):
        super().__init__()
        self.action_dim = action_dim
        self.feat_dim = feat_dim
        ...
        self.position_encoding = PositionalEncoding1D(max_length=max_length, embed_dim=feat_dim)
        self.layer_stack = nn.ModuleList([
            AttentionBlockKVCacheProgressive(feat_dim=feat_dim, hidden_dim=feat_dim*2, num_heads=num_heads, dropout=dropout) for _ in range(num_layers)
        ])
        self.layer_norm = nn.LayerNorm(feat_dim, eps=1e-6)  # TODO: check if this is necessary
    ...
    def forward_with_kv_cache(self, samples, action):
        ...
        '''
        assert samples.shape[1] == 1
        current_length = self.kv_cache_list[0].shape[1] + 1
        if self.use_progressive_in_kv:
            if self.use_mild_decay_in_kv:
                decay_factor = 0.95
                min_weight = 0.3
            else:
                decay_factor = 0.9
                min_weight = 0.1
            distances = torch.arange(current_length - 1, -1, -1, device=samples.device, dtype=torch.float32)
            weights = torch.maximum(torch.full_like(distances, min_weight), decay_factor ** distances)
            penalties = torch.log(weights)
            penalties[-1] = 0.0  # current position
            mask = penalties.unsqueeze(0).unsqueeze(0).unsqueeze(0)  # [1,1,1,length]
        else:
            mask = get_vector_mask(current_length, samples.device)
